...
3、训练好的权值文件保存在logs文件夹中，每个训练世代（Epoch）包含若干训练步长（Step），每个训练步长进行一次梯度下降。
...
if __name__ == "__main__":
    ...
    #   resnet101
    # ---------------------------------#
    backbone = "mobilenetv2"  # 可以选择"mobilenetv2", "mobilenetv3_large", "mobilenetv3_small", "xception"
    # ----------------------------------------------------------------------------------------------------------------------------#
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = r"G:\model_code\deeplabv3-plus_ResNet_strip\model_data\deeplab_mobilenetv2.pth"
    # ---------------------------------------------------------#
    ...
    #   自定义创新点控制栏
    # -------------------------------------------------------------------#
    use_strip_pooling = False  # 设置为 True 开启条带池化，False 则回退到原始 DeepLabV3+
    # ------------------------------------------------------------------#
    #   use_axial_attention  是否使用轴向注意力机制
    #                        True: 在ASPP模块的三个空洞卷积分支添加轴向注意力
    #                        False: 不使用轴向注意力机制
    # ------------------------------------------------------------------#
    use_axial_attention = False  # 新增参数，控制是否启用轴向注意力机制
    # ==================================================================#
    #   是否使用 骨架感知 (Skeleton-aware) 权重来强化侵蚀沟的中心连通性
    # ==================================================================#
    use_skeleton_aware = False
    ...
    if local_rank == 0:
        ...
        # 打印创新点设置
        print(f"使用轴向注意力机制: {use_axial_attention}")
        print(f"使用条带池化: {use_strip_pooling}")
        print(f"使用骨架感知损失: {use_skeleton_aware}")
    ...
    if True:
        ...
        # ---------------------------------------#
        if local_rank == 0:
            start_time = time.time()  # 新增：记录训练开始时间
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            # 核心修改点：把骨架感知损失的开关传给 fit_one_epoch
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch,
                          ...
                          use_skeleton_aware=use_skeleton_aware)
        ...
        if local_rank == 0:
            ...
            print("-" * 50)
            print(f"训练结束！模型总耗时: {minutes} 分 {seconds} 秒")
            print("-" * 50)